...
export class PrometheusExecutionDataMetricsService implements PrometheusMetricsCollector {
	...
	get enabled(): boolean {
		return this.globalConfig.endpoints.metrics.includeExecutionDataMetrics;
	}

	init() {
		const readsTotal = new promClient.Counter({
			name: `${this.globalConfig.endpoints.metrics.prefix}execution_data_reads_total`,
			help: 'Total number of execution data reads.',
			labelNames: ['mode', 'result'],
		});

		const writesTotal = new promClient.Counter({
			name: `${this.globalConfig.endpoints.metrics.prefix}execution_data_writes_total`,
			help: 'Total number of execution data writes.',
			labelNames: ['mode', 'result'],
		});

		const unreadableBundlesTotal = new promClient.Counter({
			name: `${this.globalConfig.endpoints.metrics.prefix}execution_data_unreadable_bundles_total`,
		...
		const readDurationHistogram = new promClient.Histogram({
			name: `${this.globalConfig.endpoints.metrics.prefix}execution_data_read_duration_seconds`,
		...
		const writeDurationHistogram = new promClient.Histogram({
			name: `${this.globalConfig.endpoints.metrics.prefix}execution_data_write_duration_seconds`,
		...
		const storageModeGauge = new promClient.Gauge({
			name: `${this.globalConfig.endpoints.metrics.prefix}execution_data_storage_mode`,
