	...
	implements PrometheusMetricsCollector
{
	...
	get enabled(): boolean {
		return this.globalConfig.endpoints.metrics.includeWorkflowExecutionDuration;
	}
	...
	init() {
		...
		const durationHistogram = new promClient.Histogram({
			name: `${this.globalConfig.endpoints.metrics.prefix}workflow_execution_duration_seconds`,
