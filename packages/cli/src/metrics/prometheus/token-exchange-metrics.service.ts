...
export class PrometheusTokenExchangeMetricsService implements PrometheusMetricsCollector {
	...
	init() {
		const tokenExchangeRequestsTotal = new promClient.Counter({
			name: `${this.globalConfig.endpoints.metrics.prefix}token_exchange_requests_total`,
		...
		const tokenExchangeFailuresTotal = new promClient.Counter({
			name: `${this.globalConfig.endpoints.metrics.prefix}token_exchange_failures_total`,
			help: 'Total number of token exchange failures broken down by reason.',
			labelNames: ['reason'],
		});

		const embedLoginRequestsTotal = new promClient.Counter({
			name: `${this.globalConfig.endpoints.metrics.prefix}embed_login_requests_total`,
		...
		const embedLoginFailuresTotal = new promClient.Counter({
			name: `${this.globalConfig.endpoints.metrics.prefix}embed_login_failures_total`,
		...
		const tokenExchangeJitProvisioningTotal = new promClient.Counter({
			name: `${this.globalConfig.endpoints.metrics.prefix}token_exchange_jit_provisioning_total`,
		...
		const tokenExchangeIdentityLinkedTotal = new promClient.Counter({
			name: `${this.globalConfig.endpoints.metrics.prefix}token_exchange_identity_linked_total`,
