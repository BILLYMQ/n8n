...
describe('PrometheusWorkflowExecutionDurationMetricsService', () => {
	...
	beforeEach(() => {
		...
		service = new PrometheusWorkflowExecutionDurationMetricsService(globalConfig, eventService);
	...
	describe('enabled', () => {
		it('should be true when includeWorkflowExecutionDuration is true', () => {
			globalConfig.endpoints.metrics.includeWorkflowExecutionDuration = true;
			expect(service.enabled).toBe(true);
		});

		it('should be false when includeWorkflowExecutionDuration is false', () => {
			globalConfig.endpoints.metrics.includeWorkflowExecutionDuration = false;
	...
	describe('workflow-post-execute event handler', () => {
		...
		it('should compute duration correctly: (stoppedAt - startedAt) / 1000', () => {
			...
			expect(promClient.Histogram.prototype.observe).toHaveBeenCalledWith(expect.any(Object), 60);
		});
	});
});
